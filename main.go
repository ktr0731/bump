...
var (
	...
	ver = version.Must(version.NewSemver("0.1.0"))
)
...
// such that `version.NewSemver("0.3.4")`.
...
func findVersionExpr(fset *token.FileSet, f *ast.File) (ast.Expr, error) {
	...
	ast.Inspect(f, func(n ast.Node) bool {
		...
		if selExpr.Sel.Name != "NewSemver" {
			return true
		}

		if len(expr.Args) != 1 {
			err = errors.New("number of version.NewSemver args must be one")
			return false
		}
...
func processExpr(e ast.Expr, typ bumpType) (ver *version.Version, err error) {
	switch l := e.(type) {
	case *ast.BasicLit:
...
func processBasicLit(l *ast.BasicLit, typ bumpType) (*version.Version, error) {
	if l.Kind != token.STRING {
		return nil, errors.Errorf("arg of version.NewSemver must be string literal, passed %T", l.Kind)
	}
	...
	ver, err := version.NewSemver(sv)

	segs := ver.Segments()
	switch typ {
	case bumpTypeMajor:
		ver, err = version.NewSemver(fmt.Sprintf("%d.0.0", segs[0]+1))
	case bumpTypeMinor:
		ver, err = version.NewSemver(fmt.Sprintf("%d.%d.0", segs[0], segs[1]+1))
	case bumpTypePatch:
		ver, err = version.NewSemver(fmt.Sprintf("%d.%d.%d", segs[0], segs[1], segs[2]+1))
	case bumpTypeNoop:
		// No-op
	...
	if err != nil {
		return nil, errors.Errorf("failed to bump up version: %s", err)
	}
...
func processObject(o *ast.Object, typ bumpType) (*version.Version, error) {
